...
const renderRat = (
	...
	// Variable loops
	const loops = updated.matchAll(/!%([a-zA-Z0-9_]*?)% \*([a-zA-Z0-9_]*)\* ?{\s*(.*)\s*}/g);
	...
	while (!(next = loops.next()).done) {
		...
		for (const elem of opts[next.value[1]]) {
			replaceString += next.value[3].replace(new RegExp(`%${next.value[2]}%`, 'g'), elem) + '\n';
		}
	...
	do {
		last = updated;
		updated = updated.replace(/\{\.([a-zA-Z0-9-_]+)([^]*?)(?:\}|(?:\{[^]*?\})[^]*\})/g, `<div class="$1">$2</div>`);
		updated = updated.replace(
			/\|\.([a-zA-Z0-9-_]+)([^]*?)(?:\||(?:\|[^]*?\|)[^]*\|)/g,
	...
	updated = updated.replace(/((<li class="oli">.*<\/li>\s*)+)/g, '<ol>$1</ol>');

	// Videos
	updated = updated.replace(/\[{2}(.*)\]{2}/g, `<div><video><source src="$1"/></video></div>`);
	...
	let cssText = '';
	if (!cssFile) {
		const text = fs.readFileSync(path.join(__dirname, './rat-def.css'), 'utf8');
		cssText += text;
		cssFile = 'none';
	}
	cssText +=
...
const enableRat = (app: Express): void => {
	app.engine('rat', (path: string, opts: { [option: string]: any }, callback: (e: any, rendered?: string) => any) => {
		fs.readFile(path, (err: NodeJS.ErrnoException | null, content: Buffer) => {
			if (err) return callback(err);
			const passedOptions: { [option: string]: string } = {};
			for (const opt of Object.keys(opts)) {
				if (opt !== 'settings' && opt != 'cache' && opt !== '_locals') passedOptions[opt] = opts[opt];
			}
			const rendered = renderRat(content.toString(), passedOptions, callback);
			if (!rendered) return;
